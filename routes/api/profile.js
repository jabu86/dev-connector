...
router.get('/github/:username', (req, res) => {
    return res.send('Github repos');
    try {
        const options = {
            uri: `http://api/github.com/users/${req.param.username}/repos?per_page=5&
            sort=created:asc&client_id=${config.get('githubClentId')}&
        ...
        if (response.statusCode !== 200) {
            return res.status.apply(404).json({msg:'No Github profile found'})
        }
